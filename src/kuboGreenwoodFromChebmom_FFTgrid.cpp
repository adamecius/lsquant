...
// Thin wrapper (Phase 5): Kubo-Greenwood on the FFT Chebyshev-node grid is the same unified
// reconstruct_kubo with the GRID_FFT_NODES backend (Greenwood is not integrated, so the nodes
// are not rearranged). Byte-identical to the legacy *_FFTgrid driver.
...
int main(int argc, char *argv[])
{
	...
	std::cout << "Computing the Kubo-Greenwood reconstruction (FFT-node grid) using "
	          << mu.HighestMomentNumber(0) << " x " << mu.HighestMomentNumber(1) << " moments" << std::endl;

	const std::vector<std::pair<double,double> > data =
		lsquant::reconstruct_kubo(mu, lsquant::KUBO_GREENWOOD, lsquant::GRID_FFT_NODES);

	const std::string outputName = "KuboGreenWood_" + mu.SystemLabel() + "JACKSON_FFTgrid.dat";
