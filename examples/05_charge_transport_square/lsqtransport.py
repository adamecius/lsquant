...
def main():
    ...
    fig, (axA, axB) = plt.subplots(2, 1, figsize=(6.4, 6.6), sharex=True)
    for (W, (t, D)), c in zip(curves.items(), colors):
        axA.plot(t, D, color=c, lw=1.6, label="W = %g" % W)
        axB.plot(t, D / D.max(), color=c, lw=1.6, label="W = %g" % W)
    axA.set_ylabel(r"running diffusion $D(E,t)=\Delta X^2/2t$")
    axA.set_title(r"Square Anderson lattice, $E=2t$: ballistic $\to$ diffusive $\to$ localized")
    axA.legend(title="disorder", fontsize=9)
    axB.set_ylabel(r"normalised $D(E,t)/D_{\max}$")
    axB.set_xlabel("time $t$ (fs)")
    axB.axhline(1.0, color="0.85", lw=0.8)
    axB.text(0.5 * TMAX, 1.02, "rising = ballistic; flat = diffusive; falling = localizing",
             fontsize=8.5, color="0.3", ha="center")
    fig.tight_layout(); fig.savefig("fig_diffusion.png", dpi=150)
    print("wrote fig_diffusion.png")

    # figure 2: conductivity and mean free path vs disorder (log-y to span the decades)
    fig, ax = plt.subplots(figsize=(6.4, 4.2))
    ax.semilogy(W_LIST, sigma, "o-", color="#1f77b4", label=r"$\sigma=e^2\rho D_{\max}$")
    ax.set_xlabel("disorder $W/t$"); ax.set_ylabel(r"conductivity $\sigma$ (arb.)", color="#1f77b4")
    ax.tick_params(axis="y", labelcolor="#1f77b4")
    ax2 = ax.twinx()
    ax2.semilogy(W_LIST, ell, "s--", color="#d62728", label=r"$\ell=2D_{\max}/v_F$")
    ax2.axhline(1.0, color="#d62728", lw=0.7, ls=":")
    ax2.set_ylabel(r"mean free path $\ell$ (a)", color="#d62728")
    ax2.tick_params(axis="y", labelcolor="#d62728")
    ax.set_title(r"Conductivity and mean free path shrink with disorder ($E=2t$)")
    fig.tight_layout(); fig.savefig("fig_transport.png", dpi=150)
